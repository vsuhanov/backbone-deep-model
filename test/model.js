// yanked from Chaplin.0.9.10 test suite
define(['chaplin', 'jquery', 'underscore', 'QUnit', 'env'], function (Chaplin, $, _, QUnit, Environment)
{
	return {
		run : function ()
		{
			// test Chaplin.DeepModel instead of Chaplin.Model - reset at end of function
			var oldModel = Chaplin.Model;
			Chaplin.Model = Chaplin.DeepModel;
			var proxy = Chaplin.Model.extend();
			var klass = Chaplin.Collection.extend({
													  url : function ()
													  {
														  return '/collection';
													  }
			...
			module("Chaplin.Model", _.extend(new Environment, {
			...
			{
				var a = new Chaplin.Model({ 'foo' : 1, 'bar' : 2, 'baz' : 3});
				var b = a.clone();
				equal(a.get('foo'), 1);
				equal(a.get('bar'), 2);
				equal(a.get('baz'), 3);
				...
				var foo = new Chaplin.Model({p : 1});
				var bar = new Chaplin.Model({p : 2});
			...
			test("isNew", 6, function ()
			{
				...
				a = new Chaplin.Model({ 'foo' : 1, 'bar' : 2, 'baz' : 3, 'id' : -5 });
				ok(!a.isNew(), "any defined ID is legal, negative or positive");
				a = new Chaplin.Model({ 'foo' : 1, 'bar' : 2, 'baz' : 3, 'id' : 0 });
				ok(!a.isNew(), "any defined ID is legal, including zero");
				ok(new Chaplin.Model({          }).isNew(), "is true when there is no id");
				ok(!new Chaplin.Model({ 'id' : 2  }).isNew(), "is false for a positive integer");
				ok(!new Chaplin.Model({ 'id' : -5 }).isNew(), "is false for a negative integer");
			});
			...
			{
				var a = new Chaplin.Model({id : 'id', foo : 1, bar : 2, baz : 3});
				var changeCount = 0;
			...
			{
				var model = new Chaplin.Model({result : 'result'});
				model.on('change', function ()
				{
			...
			test("multiple unsets", 1, function ()
			{
				...
				var model = new Chaplin.Model({a : 1});
			...
			{
				var MongoModel = Chaplin.Model.extend({idAttribute : '_id'});
			...
			{
				var model = new Chaplin.Model({name : "Model"});
				model.set({name : ''});
			...
			{
				var changed;
				var model = new Chaplin.Model({id : 1, name : "Model"});
			...
			{
				var Defaulted = Chaplin.Model.extend({
														 defaults : {
															 "one" : 1,
															 "two" : 2
														 }
				...
				Defaulted = Chaplin.Model.extend({
													 defaults : function ()
													 {
														 return {
															 "one" : 3,
			...
			{
				var model = new Chaplin.Model({name : "Tim", age : 10});
			...
			{
				var model = new Chaplin.Model({a : 'a', b : 'b'});
				deepEqual(model.changedAttributes(), false);
			...
			{
				var value;
				var model = new Chaplin.Model({name : 'Rob'});
			...
			test("change after initialize", 1, function ()
			{
				...
				var obj = new Chaplin.Model(attrs);
			...
			{
				var env = this;
				var model = new Chaplin.Model({firstName : "Taylor", lastName : "Swift"});
				model.url = '/test';
				model.on('change', function ()
				{
					model.save();
			...
			{
				var lastError, model = new Chaplin.Model();
			...
			test("destroy", 3, function ()
			{
				...
				var newModel = new Chaplin.Model;
			...
			{
				var error;
				var model = new Chaplin.Model({name : "One"});
				model.validate = function (attrs)
				{
					if (!attrs.name) {
						error = true;
			...
			{
				var Parent = Chaplin.Model.extend({
			...
			{
				var changed = 0, model = new Chaplin.Model({a : null});
				model.on('change', function ()
				{
			...
			{
				var model = new Chaplin.Model({x : 1});
			...
			{
				var model = new Chaplin.Model({x : undefined});
				model.on('change:x', function ()
				{
					ok(false);
				});
			...
			{
				var model = new Chaplin.Model({0 : true, '' : true});
			...
			{
				var changed = 0;
				var model = new Chaplin.Model({x : 1, y : 2});
				model.url = '/test';
				model.on('change:x', function ()
				{
					changed++;
			...
			{
				var model = new Chaplin.Model({id : 1});
				model.url = '/test';
				model.on('sync', function ()
				{
					ok(true);
				});
				Chaplin.ajax = function (settings)
				{
					settings.success();
				};
				model.fetch();
			...
			{
				var model = new Chaplin.Model({a : {key : 'value'}});
			...
			{
				var model = new Chaplin.Model({valid : true});
				model.validate = function (attrs)
				{
